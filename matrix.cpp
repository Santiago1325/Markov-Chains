...
void Matrix::round_elements(){
	for(int i = 0; i < rows; i++){
		for(int j = 0; j < columns; j++){
			a[i][j] = round((a[i][j]*1000.0))/1000.0;
		}
	}
}
...
void Matrix::set_element(int i, int j, double x){
	if(i > 0 && j > 0 && j <= columns && i <= rows){
		a[i-1][j-1] = x;
	}else{
		throw runtime_error("set_element: Index out ouf bounds");
	}
}
...
double Matrix::sum_elements_row(int row){
	double sum = 0;
	for(int j = 0; j < columns; j++){
		sum += a[row-1][j];
	}
